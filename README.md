...
| 4. 模型与测试 | `FakeModel` 全流程可用；真实模型通过 `ModelClient` 协议独立接入，Loop 内无厂商逻辑；24 个单元测试（含题目要求的六类异常场景）+ 公开测试全部通过 |

额外完成两个拓展任务：并行执行互不依赖的只读工具、过长工具输出截断（见第六节）。
...
2. **处理过长的工具输出**：`max_tool_chars`（默认 8000）截断工具结果并附 `[truncated N chars]` 提示，避免单个工具把上下文撑爆。

其余方向（Todo/Planning、Hooks、会话持久化、MCP、TUI、任务集评估）未实现，见第八节。
